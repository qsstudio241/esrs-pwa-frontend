...
// Componente Checklist principale
const Checklist = () => {
  return (
    <div>
      <h1>Checklist Component</h1>
      {/* Il tuo componente Checklist qui */}
    </div>
  );
};
...
export class LocalFsProvider {
  ...
  async initAuditTree(arg, categories = []) {
    ...
    const evidenzeDir = await this.directoryHandle.getDirectoryHandle(
      "Evidenze",
      { create: true }
    );
  ...
  async saveEvidence(arg1, file, categoryOpt, opts = {}) {
    ...
    const categoryDir = await evidenzeDir.getDirectoryHandle(category, {
      create: true,
    });
    ...
    if (exists) {
      if (options.onConflict === "skip") {
        return {
          name: fileName,
          type: file.type || "",
          path: `Evidenze/${category}/${fileName}`,
          skipped: true,
        };
      }
    ...
    const fileHandle = await categoryDir.getFileHandle(finalName, {
      create: true,
    });
    ...
    return {
      name: finalName,
      type: file.type || "",
      path: `Evidenze/${category}/${finalName}`,
    };
  }
  ...
  async saveExport(payload) {
    if (!this.ready()) throw new Error("Directory non selezionata");
    const exportDir = await this.directoryHandle.getDirectoryHandle("Export", {
      create: true,
    });
...
// Database utilities for persistent handles
